  ...
  return (
    <div>
      ...
      <p className="text-xs text-[#8A857A] mb-6">
        {isAdmin
          ? "Publiées ici, elles apparaissent automatiquement sur le site public."
          : "Seul le super administrateur peut publier une actualité sur le site public."}
      </p>
          ...
          <div key={n.id} className="card-hover bg-white border border-[#E3DCC8] rounded-sm overflow-hidden">
            ...
            <div className="p-5">
              <div className="flex items-start justify-between gap-3 mb-1">
                ...
                {isAdmin && (
                  <button onClick={() => remove(n.id)} className="text-[11px] text-[#A8332B] shrink-0">
                    Supprimer
                  </button>
                )}
              </div>
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-sm w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          />
          <div>
            <label className="block text-xs uppercase tracking-wide text-[#8A857A] mb-1">Photo (optionnelle)</label>
