    ...
    <div>
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        ...
        <div className="flex gap-3 items-center">
          ...
          <a href="/dashboard/agenda" className="text-sm border border-[#D8D0BC] px-4 py-2 rounded-sm">
            Voir l'agenda
          </a>
              ...
              {sortTasks(scoped.filter((t) => t.status === status)).map((t) => (
                <div key={t.id} className="card-hover bg-white border border-[#E3DCC8] rounded-sm p-3">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <span
                      className="text-[10px] uppercase tracking-wide px-1.5 py-0.5 rounded-sm shrink-0"
                  ...
                  {(t.due_date || t.due_time) && (
                    <p className="text-[11px] text-[#B8862E] mt-1">
                      Échéance : {t.due_date ? frDate(t.due_date) : ""} {t.due_time ? t.due_time.slice(0, 5) : ""}
                    </p>
                  )}
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-sm w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          />
          <div>
            <label className="block text-xs uppercase tracking-wide text-[#8A857A] mb-1">Priorité</label>
            <div className="flex gap-2">
              {Object.entries(priorityMeta).map(([k, v]) => (
                <button
                  ...
                  className="flex-1 text-xs py-2 rounded-sm border"
                  style={
                    priority === k
          ...
          <div className="flex gap-2">
            <div className="flex-1">
              <label className="block text-xs uppercase tracking-wide text-[#8A857A] mb-1">Date d'échéance</label>
              <input type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} className="w-full border border-[#D8D0BC] rounded-sm px-3 py-2 text-sm" />
            </div>
            <div className="flex-1">
              <label className="block text-xs uppercase tracking-wide text-[#8A857A] mb-1">Heure (optionnel)</label>
              <input type="time" value={dueTime} onChange={(e) => setDueTime(e.target.value)} className="w-full border border-[#D8D0BC] rounded-sm px-3 py-2 text-sm" />
            </div>
          </div>
          {isAdmin && (
            <select
